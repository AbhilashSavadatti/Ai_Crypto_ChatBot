...
function ChatBot() {
  ...
  return (
    <div className="chatbox blur-background large-shadow shadow-2xl shadow-white z-50 bg-[#000518] bg-opacity-70 w-[90vw] sm:w-[80vw] md:w-[70vw] lg:w-[40vw] h-[85vh] max-h-[90vh] rounded-3xl p-4">
      <div className="h-[13%] pl-3 border-gray-700 flex flex-col gap-x-4 items-center">
        <div className="mt-4 mx-auto text-center text-lg font-semibold items-center">
          <p className="max-w-lg text-2xl md:text-3xl font-semibold leading-normal text-gray-900 dark:text-white">
            Al-Chat Bot
          </p>
        </div>
        <div className="mx-auto text-center text-lg font-semibold items-center">
          <p className="text-xs md:text-sm text-gray-400">Real-time crypto market data</p>
        </div>
      </div>
        ...
        {responses.length ? (
          <div className="flex flex-col py-5 px-5 overflow-y-auto h-full custom-scrollbar">
            {responses.map((item, index) =>
              item.role === "user" ? (
                <div className="self-end mb-2 mt-2" key={index}>
                  <PromptMessage message={item.message} />
                </div>
              ) : (
                <div className="self-start mt-2 mb-2" key={index}>
          ...
          <div className="w-full h-full flex flex-col justify-center items-center">
            <div className="mt-4 w-full h-full flex flex-col justify-center items-center">
              <img
                className="rounded-md w-[100px] h-[100px] sm:w-[120px] sm:h-[120px] md:w-[150px] md:h-[150px] mx-auto"
